...
try:
    ...
    print(
        "WARN: Check dependencies. See https://github.com/jasonacox/tinytuya/issues/377"
    )
    print(f"WARN: Error: {impErr.args[0]}.")
...
class AuthNS:
    ...
    def request(self, method: str, url: str, headers, **kwargs) -> requests.Response:
        """Make a request."""
...
class Auth:
    ...
    async def request(
        self, websession: ClientSession, url: str, headers, **kwargs
    ) -> ClientResponse:
        """Make a request."""
...
########################################################
class Cloud:
    """Define the CLoud Module for tuya API Access."""
    ...
    async def async_scraper(self, url: str, headers):
        """Download data from tuya API and return the response."""
        try:
            async with aiohttp.ClientSession() as session:
                auth = Auth()

            # This will fetch data from http://example.com/api/lights
            response = await auth.request(session, url, headers)
            print("HTTP response status code", response.status)
            print("HTTP response JSON content", await response.json())
    ...
    async def _tuyaplatform(
        ...
        if action == "GET":
            response = await self.async_scraper(url, headers)
            # assert response
            # auth = AuthNS()
            # response = auth.request("get", url, headers)
    ...
    def _getuid(self, deviceid=None):
        ...
        if not self.token:
            return self.error
        if not deviceid:
            return self.error_json(ERR_PARAMS, "_getuid() requires deviceID parameter")
    ...
    def get_statusPower(self):
        ...
        remote_list = self.cloudrequest(url2)
    ...
    def getdevices(self, verbose=False, oldlist=[], include_map=False):
        ...
        if self.apiDeviceID and self.use_old_device_list:
            ...
            for dev_id in self.apiDeviceID.split(","):
                ...
                if isinstance(uid, dict):
                    # it's an self.error_json dict
                    return uid
                else:
                    uid_list[uid] = True

            if not uid_list:
                return self.error_json(ERR_CLOUD, "Unable to get uid for device list")
        ...
        if verbose:
            ...
            return self.error_json(ERR_CLOUD, "Unable to get device list")
        ...
        for dev in devs:
            ...
            if "key" not in old or old["key"] != dev["local_key"]:
                ...
                changed_devices.append(dev)
                continue
            if (("icon" not in old) and ("icon" in dev)) or (
                include_map and ("mapping" not in old or old["mapping"] is None)
            ):
            ...
            for k in DEVICEFILE_SAVE_VALUES:
                if (
                    ...
                    and k != "icon"
                    and k != "last_ip"
                    and (k not in old or old[k] != dev[k])
                ):
                    is_same = False
                    break
            if not is_same:
    ...
    def _getdevice(self, param="status", deviceid=None):
        if not self.token:
            return self.error
        if not deviceid:
            return self.error_json(ERR_PARAMS, "Missing DeviceID Parameter")
    ...
    def sendcommand(self, deviceid=None, commands=None, uri="iot-03/devices/"):
        ...
        if (not deviceid) or (not commands):
            return self.error_json(
                ERR_PARAMS, "Missing DeviceID and/or Command Parameters"
            )
    ...
    def getmapping(self, productid, deviceid=None):
        ...
        if deviceid:
            ...
            if result:
                ...
                elif ("code" in result and result["code"] == 2009) or (
                    "msg" in result and result["msg"] == "not support this device"
                ):
