...
from homeassistant.components.iraircon.IRaircon import cloud
...
def async_setup_entry(
    ...
    confipaddr = entry.data["ipaddr"]
    confApiKey = entry.data["apikey"]
    confApiSecret = entry.data["apisecret"]
    daikin_api = entry.entry_id
    # testapi = iraircon.IRAirCon(
    #     confDeviceID,
    #     confipaddr,
    #     confApiRegion,
    #     confApiKey,
...
class DaikinIRClimate(ClimateEntity):
    ...
    def __init__(self, entryid, irapi: cloud.Cloud, remoteID) -> None:
        """Initialize the climate device."""
        self._remoteID = remoteID
        self._irapi = irapi
        # self._irapi._connect()
        self._attr_current_temperature = self._irapi.getstatuspower()
        self._attr_fan_modes = self._irapi.get_fanRates()
        # self._attr_device_info = self._irapi.getstatusall
